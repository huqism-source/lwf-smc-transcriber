...
//
// youtubei.js YouTube ke internal (InnerTube) API ko seedha mimic karta hai — Vercel jaisay
// datacenter IPs se aane wali "Sign in to confirm you're not a bot" blocking ke khilaf
// zyada mazboot hai. Kayi mobile-app clients (ANDROID/IOS) try kiye jate hain taake koi ek chal jaye.
//
// IMPORTANT: require("youtubei.js") jaan-boojh kar handler ke ANDAR, apne try/catch mein
// kiya gaya hai — agar Vercel pe package load/bundle na ho paye, to poora function crash
// (FUNCTION_INVOCATION_FAILED) hone ki bajaye ek saaf JSON error wapis milega.
//
// Optional Vercel Environment Variables:
//   MAX_DURATION_SEC (default 2400 = 40 minute)
//   YT_COOKIE (apne YouTube account ka cookie header string — bot-block bypass ke liye, SETUP.md dekhein)
//
...
function json(res, status, payload) {
  try {
    ...
    // response already sent/closed — kuch nahi kar saktay
  }
}
...
let ytPromise = null;
function getYt(Innertube, UniversalCache) {
  if (!ytPromise) {
...
module.exports = async (req, res) => {
  // Sab se bahar wala safety net — kuch bhi ho, ye kabhi function ko crash nahi hone dega.
  try {
    ...
    // Package ko handler ke ANDAR load karain, try/catch ke sath — agar ye load
    // na ho (missing dependency, bundling issue, ESM/CJS masla waghera) to
    // saaf JSON error milega, function crash nahi hoga.
    let Innertube, UniversalCache, Readable;
    try {
      ...
      return json(res, 500, {
        ...
        message: "Server par youtubei.js package load nahi hui. package.json mein dependency check karein aur redeploy karein.",
    ...
    let yt;
    try {
      yt = await getYt(Innertube, UniversalCache);
    } catch (err) {
      return json(res, 502, {
        error: "init_failed",
    ...
    // Yahan tak error aana matlab kuch bilkul anokha crash — phir bhi crash
    // hone ke bajaye JSON wapis bhejain.
    if (!res.headersSent) {
      json(res, 500, {
        error: "unexpected",
